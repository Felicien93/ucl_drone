...
std::vector< cv::Point_< T > > Points(const std::vector< cv::KeyPoint >& keypoints)
{
  ...
  {
    result.push_back(cv::Point_< T >(keypoints[i].pt.x, keypoints[i].pt.y));
  }
  return result;
}
//
// void PointCloud(const std::vector< cv::KeyPoint >& keypoints, const cv::Mat descriptors,
...
//     {
//       point.descriptor[k] = descriptors.at< float >(i, k);
//     }
