  ...
  else
  {
      ...
      for (unsigned k = 0; k < knn_matches.size(); k++)
      {
        if (knn_matches[k][0].distance / knn_matches[k][1].distance < 0.9)
        {
          if (knn_matches[k][0].distance < 200.0)
          {
            ...
            pcl::PointXYZRGBSIFT pcl_point = map.cloud->points[map_idx[knn_matches[k][0].trainIdx]];
    ...
    else
    {
      ...
      for (unsigned k = 0; k < simple_matches.size(); k++)
      {
        ...
        {
          std::pair< int, int > p(map_idx[simple_matches[k].trainIdx], simple_matches[k].queryIdx);
          idx_matching_points.push_back(p);

          cv::Point3f map_point;
          pcl::PointXYZRGBSIFT pcl_point = map.cloud->points[map_idx[simple_matches[k].trainIdx]];
          map_point.x = pcl_point.x;
          map_point.y = pcl_point.y;
          map_point.z = pcl_point.z;
          map_matching_points.push_back(map_point);
...
{
  pointcloud->points.resize(idx_points.size());
                         ...
                         pcl::PointCloud< pcl::PointXYZRGBSIFT >::Ptr& pointcloud)
{
  ...
  convertToPcl(idx_points, points_out, pointcloud);
}
