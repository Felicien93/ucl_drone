...
{
  // Altitude
  //  double h = pose.z;  // image->navdata.altd;

  // Conversion from 10^-3 degrees to radians
  double yaw = -pose.rotZ;   //(double)image->navdata.rotZ / 1000 / 180 * PI;
  double pitch = pose.rotY;  //(double)image->navdata.rotY / 1000 / 180 * PI;
  double roll = pose.rotX;   //(double)image->navdata.rotX / 1000 / 180 * PI;

  cv::Mat world2drone = rollPitchYawToRotationMatrix(roll, pitch, yaw);
  cv::Mat drone2cam = rollPitchYawToRotationMatrix(PI, 0, -PI / 2);
  T = drone2cam * world2drone;  // result : world to cam
...
{
  cv::Mat d = (cv::Mat_< double >(3, 1) << (pixelx - IMG_CENTER_x) / FOCAL_LENGTH_x,
               (pixely - IMG_CENTER_y) / FOCAL_LENGTH_y, 1);

  cv::Mat temp = n_cam.t() * d;
  double t = -h / (temp.at< double >(0, 0));
  // ROS_DEBUG("t = %f (%dx%d)", t, temp.rows, temp.cols);

  cv::Mat p_cam = d * t;
  p_world = T.t() * p_cam;
}

// // converts all points in a ProcessedImage into map points in PCL format
...
// {
//   // ROS_DEBUG("IMAGE %dx%d", image->cv_ptr->image.rows, image->cv_ptr->image.cols);
//   cv::Mat T;
//   cv::Mat n_cam;
//   cv::Mat translation;
//   double pose.z;  // h = 0.76;  //
//
//   projection2D_utils1(h, pose, T, n_cam, translation);
//
...
//
//     cv::Mat p_world;
//     projection2D_utils2(pixelx, pixely, h, T, n_cam, translation, p_world);
//
//     int rgb_ = 255 << 16 | 0 << 8 | 0;  // red
//     pcl::PointXYZRGBSIFT point;
//     point.x = p_world.at< double >(0, 0) + pose.x;
//     point.y = p_world.at< double >(1, 0) + pose.y;
//     point.z = p_world.at< double >(2, 0);
//     point.rgb = *reinterpret_cast< float * >(&rgb_);
//     for (int k = 0; k < 128; k++)  // 128 is the size of a SIFT descriptor
//     {
//       point.descriptor[k] = image->descriptors.at< float >(i, k);
//     }
//     pointcloud->push_back(point);
//
//     // ROS_DEBUG("POINTS(:,%d) = [%f; %f];", i + 1, p_world.at< double >(0, 0),
